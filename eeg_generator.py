...
class EEGGenerator:
    ...
    def pick_candidate_from_mri(self, region: str):  # ex) 'white', 'pial'
        ...
        # Because this is for designating all the "candidate", I want both floor/ceiling of vertices to be masked.
    ...
    def select_dipoles_in_ras(self, n_sources: int, extents=(21/2, 58/2)):  # mm
    ...
    def dipoles_in_src(self, n_sources: int, extents=(21/2, 58/2)):  # mm
        src = self.src
        total_candidate = self.candidate
        selected_center_dipole = np.random.choice(np.arange(len(total_candidate)), size=n_sources, replace=False)
        selected_center_dipole = total_candidate[selected_center_dipole]
    ...
    def create_labels(self, n_sources: int, extents=(21/2, 58/2), dipoles_vertex_coor_dis_paired_src=None):  # mm
        if self.srctype == 'surface':
            labels = [[], []]
    ...
    def create_raw(self, labels, amplitude_kernel, time_course, events=None):
        if events is None:
            events = np.zeros((1, 3), int)
            events[:, 0] = self.sfreq * np.arange(1)
            events[:, 2] = 1

        # 항상 source_simulator 로컬 변수로 설정
        if getattr(self, 'source_simulator', None) is None:
            self.source_simulator = self.create_src_simulator()
    ...
    def generate_eeg(self, raw, snr_db, filter=None):
        raw1 = raw.copy()
        snr = 10**(snr_db/10)
...
def ras_to_voxel(ras_coords, affine):
    """RAS coordinates to voxel coordinates conversion"""
